...
public class AsyncRedisParser {
    ...
    private RedisObject readObject(byte[] bytes, int[] i) {
        ...
        return switch (bytes[i[0]++]) {
            case '+' -> parseSimpleString(bytes, i);
            case '-' -> parseError(bytes, i);
            case ':' -> parseInteger(bytes, i);
            case '$' -> parseBulkString(bytes, i);
            case '*' -> parseArray(bytes, i);
            default -> throw new IllegalArgumentException("Encountered unknown type marker " + ((char) bytes[i[0] - 1]));
        };
    }
    ...
    private long readLong(byte[] bytes, int[] index) {
        ...
        index[0] = end;
        return Long.parseLong(new AsciiString(bytes, false), start, end, 10);
    }
